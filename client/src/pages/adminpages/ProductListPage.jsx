...
// FILE: client/src/pages/ProductListPage.jsx (Corrected with Relative API Paths)
// FILE: client/src/pages/adminpages/ProductListPage.jsx (Completely Redesigned)
...
const ProductListPage = ({ navigate }) => {
  ...
  // State for product data and UI status
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // State for client-side search and sort functionality
  const [searchTerm, setSearchTerm] = useState('');
  const [sortKey, setSortKey] = useState('name');

  // --- DATA FETCHING ---
  // This effect fetches all products from the admin-only endpoint when the component mounts.
  useEffect(() => {
    const fetchAllProductsForAdmin = async () => {
      try {
        setLoading(true);
  ...
  }, [userInfo, navigate, showNotification]);

  // --- HANDLERS ---
  // Handles the creation of a new sample product
  const createProductHandler = async () => {
    if (window.confirm('Create a new sample product?')) {
      try {
        const res = await fetch('/api/products', {
  ...
  const deleteHandler = async (id) => {
    if (window.confirm('Permanently delete this product?')) {
      try {
        ...
        showNotification('Product deleted!', 'success');
        // Instantly update the UI by removing the product from the local state
  ...
  // --- DERIVED STATE (FILTERING & SORTING) ---
  // This logic runs on the client-side after all products are fetched.
  const filteredAndSortedProducts = [...products]
    .filter(p => 
        p && p.name &&
        p.name.toLowerCase().includes(searchTerm.toLowerCase())
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <motion.button 
            onClick={createProductHandler}
            className="bg-[#D4A28E] text-white px-5 py-3 rounded-xl hover:bg-[#C8907A] transition-all shadow-md flex items-center justify-center gap-2 font-semibold"
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.98 }}
          >
            <FiPlus />
        ...
        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <select 
            ...
            className="px-4 py-3 border border-gray-200 bg-white rounded-xl focus:ring-2 focus:ring-[#D4A28E] focus:outline-none transition"
          >
                ...
                {filteredAndSortedProducts.map((product) => (
                  ...
                  >
                    <div className="h-48 bg-gray-50 p-4">
                      <img 
                        src={(product.images && product.images[0]) || product.image || `https://placehold.co/400x400/e2e8f0/333?text=Image`} 
                        alt={product.name}
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <div className="mt-auto pt-4 flex justify-between items-center">
                        <p className="text-lg font-bold text-[#5C3A2E]">${(product.price || 0).toFixed(2)}</p>
                        <div className="flex items-center gap-2">
                          <button onClick={() => navigate(`/admin/product/${product._id}/edit`)} className="p-2 rounded-md hover:bg-gray-100 text-gray-500 transition-colors"><FiEdit /></button>
                          <button onClick={() => deleteHandler(product._id)} className="p-2 rounded-md hover:bg-red-50 text-red-500 transition-colors"><FiTrash2 /></button>
                        </div>
                      </div>
                    </div>
                  </motion.div>
